...
def o(
    ...
    if e == None:
        for k in key_list:
            ...
            assert_as( k in D, d2s("k in D? No,",qtd(k),"not in",kys(D)))
            D = D[k]
        ...
        for k in key_list[:-1]:
            k = str_to_tuple_as_necessary(k)
            if k not in D:
                if Meta['create_missing_paths']:
                    if Meta['report_implicit_path_creation']:
...
def _get_path(p):#,w,s):
    ...
    prefix = Meta['current_prefix_path']
...
def _message(message):
    Meta['messages'] = Meta['messages'][-Meta['max_num_messages']:]
    Meta['message_ctr'] += 1
    Meta['messages'].append((Meta['message_ctr'],message))
...
def _up_or_down(u,d):
    ...
    if u == 1:
        ...
        _message('went up to '+Meta['current_prefix_path'])
        return o( w=Meta['current_prefix_path'] ) ###################

    if d == 1 or d == True or has_form_of_alias(d):
        key_list = Meta['current_prefix_path'][:-1].split('/')
        D = Environment['dictionary']
        for k in key_list:
            k = str_to_tuple_as_necessary(k)
            D = D[k]
