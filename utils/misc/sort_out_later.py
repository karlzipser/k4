from k4.utils.misc.printing import *
...
def display(f,Images={},extent=-1,selected=[],notable=[]):
    from k4.utils.vis import zimread,mci,cv2,resize_to_extent
    try:
        if f not in Images:
            Images[f] = 'temp'
            try:
                ...
                from k4.utils.core.arrays import z55,rndn
...
def image_loader_thread(D={'Images':{},'done':False,'fs':[],'extent':400}):
    import keyboard
    from k4.utils.vis import zimread,resize_to_extent
    Images = D['Images']
    for f in D['fs']:
        if D['done']:
            return
